	...
	try:
					...
					try:
						...
						for relationship in AGO_RELATIONSHIPS:
							...
							for related_item in related_items:
								result = user_item.delete_relationship(related_item, relationship)
								if result:
							...
							related_items = user_item.related_items(relationship, direction='reverse')
							for related_item in related_items:
								result = related_item.delete_relationship(user_item, relationship)
								if result:
		...
		for user_folder in portal_user.folders:
			...
			if portal_user_name == user_folder['username'] :
				result = content_manager.delete_folder(folder_name, owner=portal_user_name)
				if result:
...
try:
	...
	for acct in accounts:
		...
		if args.commit:
			...
			delete_account_and_assets(acct, org_content_manager, our_licenses, args.verbose)
		else:
