...
def delete_account_and_assets( portal_user, content_manager, available_licenses, verbose ):
	...
	try:
		...
		for user_folder in portal_user.folders:
			...
			if user_folder['username'] == portal_user_name:
		...
		member_groups = portal_user.groups
		number_of_groups = len(member_groups)
		print(portal_user_name, ": Member of {} groups".format(number_of_groups))
		for org_group in member_groups:
			response = org_group.get_members()
			if portal_user_name == response['owner']:
				if verbose: print("Deleting group: ", org_group)
				outcome = org_group.delete()
				if not outcome:
					print("** FAILED to delete the group **")
			else:
				if verbose: print("Removing from group: ", org_group)
				not_removed_users = org_group.remove_users([portal_user_name])
